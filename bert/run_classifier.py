...
class MyTaskProcessor(DataProcessor):
  ...
  def __init__(self):
    self.labels = ['体育', '健康', '女人', '娱乐', '房地产', '教育', '文化', '新闻', '旅游', '汽车','科技','财经']

  def get_train_examples(self, data_dir):
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, 'sohu_train.txt')), 'train')

  def get_dev_examples(self, data_dir):
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, 'sohu_val.txt')), 'val')

  def get_test_examples(self, data_dir):
    return self._create_examples(
      self._read_tsv(os.path.join(data_dir, 'sohu_test.txt')), 'test')
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  # the entire model is fine-tune更改“_create_examples()”.
  tokens = []
  segment_ids = []
  tokens.append("[CLS]")
